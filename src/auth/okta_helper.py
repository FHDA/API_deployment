...
class OktaHelper:
    ...
    def __init__(self):
        ...
        configs.read(os.path.dirname(os.path.abspath(__file__))
                     + '/../../config/setup.cfg')
    ...
    def is_access_token_valid(self, token):
        ...
        jwt_verifier = JWTVerifier(self.okta_configs['issuer'],
                                   self.okta_configs['client_id'],
                                   'api://default')
        try:
            loop.run_until_complete(jwt_verifier.verify_access_token(token))
            (headers, claims, signing_input, signature) = \
                jwt_verifier.parse_token(token)
    ...
    def is_access_request_authorized(self, request):
        ...
        try:
            ...
            return (False,
                    'Can not get authorization token from request.')
